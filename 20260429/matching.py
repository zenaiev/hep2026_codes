...
def matching(genSign, genp4, recN, recPt, recEta, recPhi, h, genVector):
  #global genVector
  delta_best = -1
  irec_best = -1
  #genVector = None
  for irec in range(recN):
    ...
    genVector.SetPx(genp4[0])
    genVector.SetPy(genp4[1])
    genVector.SetPz(genp4[2])
    genVector.SetM(genp4[3])
    delta_phi = genVector.Phi()-recPhi[irec]
    if delta_phi > ROOT.TMath.Pi():
      delta_phi = 2 * ROOT.TMath.Pi() - delta_phi
    elif delta_phi < -1 * ROOT.TMath.Pi():
      delta_phi = 2 * ROOT.TMath.Pi() + delta_phi
...
t.Add('ntuples-mc/TTJets_TuneZ2_7TeV-madgraph-tauola/00000/ttbarSel_merged.root')
...
delta_max = 0.1
delta_max_jet = 1.0
h_jet = ROOT.TH1F('h_delta', '', 1000, 0., 10.)
h_mat_jet = ROOT.TH1F('h_delta_mat', '', 1000, 0., 10.)
...
t_new_jet = ROOT.TTree('tree_matched_jet', 'tree_matched_jet')
ttree_delta_best_jet = new_branch(t_new_jet, 'delta_best_jet', 'f')
ttree_irec_best_jet = new_branch(t_new_jet, 'irec_best_jet', 'i')
ttree_gen_pt_jet = new_branch(t_new_jet, 'gen_pt_jet', 'f')
ttree_rec_pt_jet = new_branch(t_new_jet, 'rec_pt_jet', 'f')
...
nevents = min(1000000, nevents)
genVector_el = ROOT.Math.PxPyPzMVector()
genVector_mu = ROOT.Math.PxPyPzMVector()
genVector_jet = ROOT.Math.PxPyPzMVector()
for i in tqdm.tqdm(range(nevents)):
  t.GetEntry(i)
  irec_best_el, delta_best_el, genp4_el = matching(+1, mcLp, Nel[0], elPt, elEta, elPhi, h_el, genVector_el)
  irec_best_mu, delta_best_mu, genp4_mu = matching(+1, mcLp, Nmu[0], muPt, muEta, muPhi, h_mu, genVector_mu)
  irec_best_jet, delta_best_jet, genp4_jet = matching(0, mcB, Njet[0], jetPt, jetEta, jetPhi, h_jet, genVector_jet)
  ttree_delta_best_el[0] = -1
  ttree_irec_best_el[0] = -1
  ttree_delta_best_mu[0] = -1
  ttree_irec_best_mu[0] = -1
  ttree_delta_best_jet[0] = -1
  ttree_irec_best_jet[0] = -1
  ttree_gen_pt_el[0] = -1
  ttree_rec_pt_el[0] = -1
  ttree_gen_pt_mu[0] = -1
  ttree_rec_pt_mu[0] = -1
  ttree_gen_pt_jet[0] = -1
  ttree_rec_pt_jet[0] = -1
  ...
  if delta_best_jet < delta_max_jet and irec_best_jet >= 0:
    h_mat_jet.Fill(delta_best_jet)
    ttree_delta_best_jet[0] = delta_best_jet
    ttree_irec_best_jet[0] = irec_best_jet
    ttree_gen_pt_jet[0] = genp4_jet.Pt()
    ttree_rec_pt_jet[0] = abs(jetPt[irec_best_jet])
    t_new_jet.Fill()
    h2_resp[f"h_resp_jet_pt"].Fill(genp4_jet.Pt(), jetPt[irec_best_jet])
    h2_resp[f"h_resp_jet_eta"].Fill(genp4_jet.Eta(), jetEta[irec_best_jet])
    h2_resp[f"h_resp_jet_phi"].Fill(genp4_jet.Phi(), jetPhi[irec_best_jet])

ROOT.gStyle.SetOptStat(0)
for part in particles:
  c_resp = ROOT.TCanvas(f'c_resp_{part}', 'Response matrix', 600, 600)
  c_resp.Divide(3,3)
...
c.Divide(3)
h_mat = [h_mat_el, h_mat_mu, h_mat_jet]
for ih,h in enumerate([h_el, h_mu, h_jet]):
  c.cd(ih+1)
  ROOT.gPad.SetLogx()
  ROOT.gPad.SetLogy()
  h.Draw()
